...
# Examples:
#    ./certora/tests/verifyBug_multiAsset.sh participants 2
#    ./certora/tests/verifyBug_multiAsset.sh participants 3 --rule initializeCalledOnce
...
if [ -f "$PATCH_PATH" ]; then
  ...
  certoraRun certora/conf/verifyRewardsController_multiAsset_verified.conf --send_only --msg "${MSG}" "$@" # pass all other parameters to certoraRun
  git apply -R "$PATCH_PATH"
else
  # Run without patching 
  certoraRun certora/conf/verifyRewardsController_multiAsset_verified.conf --send_only --msg "${MSG}" "$@" 
fi
